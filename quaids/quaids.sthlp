...
{cmd:help quaids}{right: ({browse "http://www.stata-journal.com/article.html?article=st0268":SJ12-3: st0268})}
{hline}
...
{opt quaids} {it:varlist_expshares} {ifin}{cmd:,} {opt anot(#)}
   ...
   {c -(}{opt exp:enditure(varname_exp)}|{opt lnexp:enditure(varname_lnexp)}{c )-}
   [{it:{help quaids##options:options}}]

{pstd}where {it:varlist_expshares} is the list of expenditure share
...
{p2coldent :* {opt prices(varlist_prices)}}list of price variables{p_end}
{p2coldent :* {opt lnprices(varlist_lnprices)}}list of variables containing natural logarithms of prices{p_end}
{p2coldent :+ {opt expenditure(varname_exp)}}variable representing total expenditure{p_end}
{p2coldent :+ {opt lnexpenditure(varname_lnexp)}}variable representing the natural logarithm of total expenditure{p_end}
...
{pstd}{cmd:quaids} estimates the parameters of Deaton and Muellbauer's
...
{phang}{opt anot(#)} specifies the value of the alpha_0 parameter that
appears in the price index.  {opt anot()} is required.

{phang}{opt prices(varlist_prices)} specifies the list of price
...
{phang}{opt lnprices(varlist_lnprices)} specifies the list of variables
...
{phang}{opt expenditure(varname_exp)} specifies the variable
...
{phang}{opt lnexpenditure(varname_lnexp)} specifies the variable
...
{phang}{opt demographics(varlist_demo)} requests that the variables
{it:varlist_demo} be included in the demand system based on the scaling
technique introduced by Ray (1983).

{phang}{opt noquadratic} requests that the quadratic income term not be
...
{synopt:{cmd:e(alpha)}}estimated alpha vector{p_end}
{synopt:{cmd:e(beta)}}estimated beta vector{p_end}
{synopt:{cmd:e(gamma)}}estimated gamma matrix{p_end}
{synopt:{cmd:e(lambda)}}estimated lambda vector (if applicable){p_end}
{synopt:{cmd:e(eta)}}estimated eta matrix (f applicable){p_end}
{synopt:{cmd:e(rho)}}estimated rho vector (if applicable){p_end}
